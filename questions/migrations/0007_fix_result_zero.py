...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        to_fix = []
        aff = set(['%s-%s' % (a, b) if a <= 10 else 'subtraction <= 20'
                   for a in range(1, 21) for b in range(1, a + 1)])
        for q in orm.Question.objects.all():
            if q.skill.name in aff:
                data = json.loads(q.data)
                if 'kb' in data and data['kb'] != 'full':
                    to_fix.append(q)
        for a in orm.Answer.objects.filter(question__in=to_fix):
            a.delete()
        for q in to_fix:
            data = json.loads(q.data)
            data['kb'] = [0] + data['kb']
